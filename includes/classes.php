...
class updateSettings {
    ...
    function vl($data) {
        ...
        if($data['token_id'] == $_SESSION['token_id']) {
            $httpClient = new GuzzleHttp\Client(['timeout' => 10/*, 'verify' => false*/]);

            try {
                $response = $httpClient->request('POST', strrev('etadilav/esnecil/1v/ipa/moc.oitanul.ipa//:sptth'),
                    [
                        'form_params' => [
                            strrev('di_esnecil') => $_POST[strrev('kl')],
                            strrev('di_tcudorp') => 1,
                            'url' => $CONF['url']
                        ]
                    ]
                ...
                if ($output->status == 200) {
                    $this->db->query(sprintf("UPDATE `settings` SET `%s` = '%s', `%s` = '%s'", strrev('kl'), $_POST[strrev('kl')], strrev('tl'), $output->type ? strrev('FmCLHjn533') : strrev('wQ8sGDbGAy')));
                }

                $_SESSION[strrev('egassem')] = notificationBox('success', $output->message);
